...
const parseManifest = async ({ elId = "islands-manifest", integrityAttr = "data-integrity", reportEvent }) => {
	...
	if (expectedIntegrity)
	{
		try
		{
			...
			{
				reportEvent({ event: "manifest_integrity_failed", detail: { expectedIntegrity, actualIntegrity } });
			}
		}
		catch (err)
		{
			reportEvent({ event: "manifest_integrity_error", detail: { message: err?.message || String(err) } });
		}
	}
...
const mountIsland = async ({ el, moduleSpecifier, props, manifest, reportEvent }) => {
	const resolved = manifest.modules[moduleSpecifier];

	if (!resolved)
	{
		reportEvent({ event: "island_module_missing", detail: { moduleSpecifier } });
		return;
	}
	...
	{
		reportEvent({ event: "island_import_failed", detail: { moduleSpecifier, resolved, message: err?.message || String(err) } });
		return;
	}
	...
	{
		try { mod.mount(el, props); }
		catch (err) { reportEvent({ event: "island_mount_failed", detail: { moduleSpecifier, resolved, message: err?.message || String(err), mode: "mount-fallback" } }); }
		return;
	}

	if (!Component)
	{
		reportEvent({ event: "island_default_export_missing", detail: { moduleSpecifier, resolved } });
		return;
	}
	...
	{
		reportEvent({ event: "island_mount_failed", detail: { moduleSpecifier, resolved, message: err?.message || String(err) } });
	}
};
 ...
 */
export const bootIslands = async ({ manifestElId = "islands-manifest", selector = "[data-island-module]", onError, reportEvent: reportEventInput } = {}) => {
	...
	if (!nodes.length)
	{
		reportEvent({ event: "islands_not_found" });
	}
	...
	for (const el of nodes)
	{
		...
		const start = () => {
			try { return mountIsland({ el, moduleSpecifier, props, manifest, reportEvent }); }
			catch (err)
			{
...
{
	if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", () => { void bootIslands(); });
	else void bootIslands();
}
